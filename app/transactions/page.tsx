  ...
  return (
    <PageTransition>
      <div>
        ...
        {/* ── Controls ── */}
<div className="mb-4">
  {/* Row 1 — time toggle + filter/search icons */}
  <div className="flex items-center justify-between gap-2">
    {/* Time toggle */}
    <div className="flex items-center gap-1 p-1 bg-surface-low rounded-lg flex-shrink-0">
      {(["daily", "monthly", "yearly"] as TimeView[]).map((v) => (
        <button
          key={v}
          onClick={() => setView(v)}
          className={`px-3 md:px-4 py-1.5 rounded-md text-xs md:text-sm font-semibold capitalize transition-all ${
            view === v
  ...
  {/* Row 2 — search bar on mobile (expands below) */}
  {showSearch && (
    <div className="mt-2 md:hidden flex items-center gap-2 bg-surface-low rounded-lg px-3 py-2 w-full">
      <Search size={14} className="text-on-surface-muted flex-shrink-0" />
      <input
        type="text"
        ...
        autoFocus
        className="flex-1 bg-transparent text-sm text-primary placeholder:text-on-surface-muted outline-none"
      />
      {search && (
        <button onClick={() => setSearch("")}>
          <X size={12} className="text-on-surface-muted" />
        </button>
      )}
    </div>
  )}

  {/* Desktop search — always visible inline */}
  <div className="hidden md:flex items-center gap-2 bg-surface-low rounded-lg px-3 py-2 mt-3">
